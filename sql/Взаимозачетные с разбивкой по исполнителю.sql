...
SELECT doc.дата as [Дата], ist.Наименование as Источник, isp.Наименование as Исполнитель, sum(opv.Сумма) as Сумма 
 from Stack.Документ doc  inner join stack.Организации ist on ist.ROW_ID = doc.[Источник-Платежи]		
inner join stack.[Список оплаты] so on so.[Платеж-Список] = doc.ROW_ID	                                
